...
class View implements IView {
	...
	static function Layout(string $layout,array $arguments=null,string $renderer='',string $encoding=''): IView {
		...
		$path = PathUtility::currentConfig(['view','layoutRootPath'], "{$layout}.php");
		$renderer = empty($renderer) ? Configuration::get('view','renderer') : $renderer;
		return new View(
		  $layout,
		  $path,
		  $arguments,
	...
	static function Template(string $controller,string $action,array $arguments=null,string $renderer='',string $encoding=''): IView {
		...
		$path = PathUtility::currentConfig(['view','templateRootPath'], $controller, "{$action}.php");
		$renderer = empty($renderer) ? Configuration::get('view','renderer') : $renderer;
		return new View(
		  "{$controller}-{$action}",
		  $path,
	...
	static function Partial(string $partial,array $arguments=null,string $renderer='',string $encoding=''): IView {
		...
		$path = PathUtility::currentConfig(['view','partialRootPath'], "{$partial}.php");
		$renderer = empty($renderer) ? Configuration::get('view','renderer') : $renderer;
		return new View(
		  $partial,
		  $path,
