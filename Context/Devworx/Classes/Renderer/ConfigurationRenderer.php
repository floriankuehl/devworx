...
class ConfigurationRenderer extends AbstractRenderer {
  ...
  public static function script(string $key,string $value,bool $module=false){
	...
	if( $module ) $attr['defer'] = null;
	return Html::element('script',$attr,'');
  }
        ...
        if( is_array($value) ){
          switch($key){
            case'head':{
			  ...
			  self::collapseManual($value,'modules','script',true);
            }break;
            case'body':{
