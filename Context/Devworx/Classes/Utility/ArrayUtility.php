...
class ArrayUtility {
	...
	/**
	 * Sets a value in a multidimensional array by a path
	 * 
	 * @param array $array The given array
	 ...
	 * @return bool
	 */
	static function set(array &$array, $value, ...$path): bool{
		...
		return self::set($array[$key], $value, ...$path);
	}
	...
	static function index(array $array,string $key,bool $group=false): array {
		...
		foreach( $array as $i => $row ){
			...
			trigger_error("Unknown array key '{$key}'",E_USER_ERROR);
			break;
		}
	...
	/**
	 * Combines a flat list of arrays
	 * 
	 ...
	 *
	 * @param array $arrays a list of arrays to be merged
	 * @return array $result the combination of all arrays
	 */
	static function merge( ...$arrays ): array {
		...
		foreach( $arrays as $other ){
			...
			foreach( $keys as $key ) {
				if( isset( $result[$key] ) 
					...
					$result[$key] = self::merge( $result[$key], $other[$key] );
				} else {
	...
	/**
	 * Removes a common prefix from a list of strings
	 * 
	 * @param array $array list
	 * @return array|string
	 */
	static function removeCommon(array $list): array {
		if (empty($list)) return [];

		$first = $list[0];
		$maxPrefixLength = strlen($first);

		for ($i = 0; $i < $maxPrefixLength; $i++) {
			$char = $first[$i];
			foreach ($list as $str) {
				if (!isset($str[$i]) || $str[$i] !== $char) {
					// Unterschied gefunden → Prefix endet hier
					$prefixLength = $i;
					break 2;
				}
			}
		...
		// Falls komplett identisch
		$prefixLength = $prefixLength ?? $maxPrefixLength;

		return array_map(
			fn($str) => substr($str, $prefixLength),
			$list
		);
	}
